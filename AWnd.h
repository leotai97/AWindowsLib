...
~Application();
 ...
 void Response(int proseTextItemID);
 DialogResult Question(String const &msg, int btns);
 DialogResult Question(int proseTextItemID, int btns);
...
class MenuItem
{
 ...
 MenuItem();
 MenuItem(MenuItem *parent, HMENU hMenu, String const &txt, int id, int pos);
 MenuItem(MenuItem *parent, String const &txt, int id, int pos);
~MenuItem();
 ...
 MenuItem *SubMenu(int id, String const &txt);
 MenuItem *SubMenu(int id);

 MenuItem *FindSubMenu(int id);
 String WalkMenuTree(int id);
 bool   WalkHandle(HMENU hMenu, int *pId);

 void Separator();
 
 void AddMenu(int id);
 void AddMenu(int id, bool checked, bool enabled, int itemData);
 void AddMenu(int id, String const &txt);
 void AddMenu(int id, String const &txt, bool checked, bool enabled, int itemData);
 ...
 bool IsSubMenu() const { return (m_IsSubMenu ==  true); }
 bool IsMenuItem() const { return (m_IsSubMenu == false); }
 bool IsSepartor() const { return (m_ID == 0); }

 void SetText(String const &text) { m_MenuText = text; }

 // have to store every type in m_Items: sub menus, separators etc because it's by pos

 std::map<int, int>      Index;   // id, position
 std::map<int, MenuItem> Items;   // pos, MenuItem
...
struct MenuItemSortByPosAsc
{
 bool operator()(MenuItem const &a, MenuItem const &b) const { return a.Position() < b.Position(); }
};

struct MenuItemSortByPosDesc
{
 bool operator()(MenuItem const &a, MenuItem const &b) const { return a.Position() > b.Position(); }
};


class AMenu : public MenuItem
{
 public:
 AMenu();
~AMenu();
 ...
 void ShowContextMenu(AWnd *parent, Point const &pt);
 void DestroyMenu();
 ...
 void SetCheckState(int id, bool checked);
 bool GetCheckState(int id);

 void SetEnabledState(int id, bool enabled);
 bool GetEnabledState(int id);

 int GetItemParam(int id);
 void SetItemParam(int id, int param);

 String GetMenuTree(int id);  // file > actions > action
 int GetHandle(HMENU hMenu);  

 String GetMenuText(int id) { return m_MenuText; }
...
~ListView();
 ...
 void AddTextColumn(String const &text, ListViewColumn::ColumnAlign align, int len);
 void AddColumn(int proseID, ListViewColumn::ColumnAlign align, int len);
...
~ADialog();
 ...
 void AddChildHandle(HWND hWnd) { m_ChildList.push_back(hWnd); }
